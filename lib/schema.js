import { affiliatedFirm, site } from '@/lib/site';
...
const areaServed = [
  ...SERVICE_CITIES.map((name) => ({ '@type': 'City', name, containedInPlace: FLORIDA })),
...
export function personSchema() {
  return {
    ...
    description:
      'Florida-licensed Certified Public Accountant with 42 years in public accounting, the last thirteen in independent practice, specializing in tax planning, compliance, tax resolution, and business advisory work.',
...
/** ItemList for the service-area hub page. */
export function serviceAreaListSchema(areaEntries) {
  return {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    '@id': `${site.url}/areas#arealist`,
    name: 'Service areas — Advance Accounting & Tax Solutions',
    itemListElement: areaEntries.map((area, index) => ({
      '@type': 'ListItem',
      position: index + 1,
      name: `${area.city}, FL`,
...
/**
 * The affiliated CPA firm. Declared separately so the two entities are not
 * conflated: this one is the CPA firm, and it is the one that can take
 * engagements requiring a CPA firm.
 */
export function affiliatedFirmSchema() {
  return {
    '@context': 'https://schema.org',
    '@type': 'AccountingService',
    '@id': `${site.url}/about#affiliated-firm`,
    name: affiliatedFirm.name,
    description: `${affiliatedFirm.summary} ${affiliatedFirm.whenItApplies}`,
    address: postalAddress,
    telephone: site.phone,
    founder: { '@id': `${site.url}/about#person` },
    employee: { '@id': `${site.url}/about#person` },
...
/** Person entries for the team page, tied to the organization. */
export function teamSchema(members) {
  return members.map((member) => ({
    '@context': 'https://schema.org',
    '@type': 'Person',
    '@id': `${site.url}/team#${member.slug}`,
    name: member.name,
